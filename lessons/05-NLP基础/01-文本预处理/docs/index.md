...
英文 NLP 的第一步是分词——按空格切开就好。绝大部分时候，`"The cats were running"` 按空格切出来的结果能用。

中文 NLP 的第一步也是分词——但中文没有空格。

"我们在北京大学学习自然语言处理"——你打算怎么切？是"北京/大学"还是"北京大学"？是"自然/语言"还是"自然语言"？选择不同，模型看到的数据就不同。

更麻烦的是，切完之后还有两道工序。英文中 `running` 和 `ran` 表达的是同一件事的不同时态——你希望模型把它们视为同一个概念还是不同概念？中文没有这种动词变位，但中文有自己的一套难题：繁体/简体转换、全角/半角混用、"的/地/得"的误用。

这就是文本预处理。它不是"洗数据"——它是整个 NLP 流水线的第一道关卡。做对了，模型事半功倍。做错了，再好的模型也救不回来。
