# main.py — 超参数调优：从零实现网格搜索、随机搜索、贝叶斯优化
# 依赖：numpy>=1.24, optuna>=3.0（可选，用于 Optuna 演示）
# 安装：pip install numpy optuna
# 对应课程：第 02 阶段 · 12（超参数调优）
...
def make_data(n_samples=400, n_features=8, seed=42):
    """生成回归数据集，按 60/20/20 切分为训练/验证/测试集。"""
...
def sample_param(spec, rng):
    ...
    - 列表：从列表中随机选一个
    - ("int", low, high)：均匀采样整数
    - ("float", low, high)：均匀采样浮点数
    - ("log_float", low, high)：在对数空间均匀采样（用于学习率等）
    """
...
class SimpleBayesianOptimizer:
    ...
    def _expected_improvement(self, mu, var, best_y):
        """期望提升（Expected Improvement）采集函数。

        在预测值高（利用）或不确定性大（探索）的位置给出高分。
        """
        sigma = np.sqrt(var)
    ...
    @staticmethod
    def _norm_cdf(x):
        """标准正态分布累积分布函数（用 tanh 近似，避免依赖 scipy）。"""
...
def demo_grid_search():
    ...
    print(f"  最优验证 neg_mse: {best_score:.4f} (MSE = {-best_score:.4f})")
...
def demo_random_search():
    ...
    for n_iter in budgets:
        ...
        elapsed = time.time() - start
        print(f"  n_iter={n_iter:>3d}  最优 MSE={-best_score:.4f}  耗时={elapsed:.1f}s")
...
def demo_bayesian():
    ...
    for k, v in best_params.items():
        if isinstance(v, float):
            ...
            print(f"    {k}: {v}")
    print(f"  最优验证 MSE: {-best_score:.4f}")
...
def demo_comparison():
    ...
    print(f"  {'评估次数':>8}  {'网格 MSE':>10}  {'随机 MSE':>10}  {'贝叶斯 MSE':>10}")
    print(f"  {'-' * 8}  {'-' * 10}  {'-' * 10}  {'-' * 10}")
    for cp in checkpoints:
        if cp < len(grid_curve):
            print(
                f"  {cp + 1:>8d}  {-grid_curve[cp]:>10.4f}  "
                f"{-rand_curve[cp]:>10.4f}  {-bayes_curve[cp]:>10.4f}"
            )

    print()
    print("随机搜索覆盖整个连续空间（覆盖更广）。")
    print("贝叶斯优化从历史结果中学习（收敛更快）。")
    print("网格搜索仅在超参数极少时有竞争力。")
    print()


def demo_optuna():
    """Optuna 工业级调优演示（需要安装 optuna）。"""
    print("=" * 60)
    print("Optuna 工业级调优")
    print("=" * 60)
    ...
    try:
        ...
        print("  未安装 Optuna。安装命令：pip install optuna")
        print("  跳过 Optuna 演示。")
        print()
        return
    ...
    for k, v in study.best_params.items():
        if isinstance(v, float):
            ...
            print(f"    {k}: {v}")
    print(f"  最优验证 MSE: {study.best_value:.4f}")
