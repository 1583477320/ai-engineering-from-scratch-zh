...
核心洞察：**Swarm 将多智能体编排归约为模型已经做的——工具调用。** 不需要新的抽象层。
...
**例程 (Routine)：** 定义智能体角色和可用工具的系统提示词。想想它就像一个限定范围的指令集："你是分诊智能体；如果用户问退款，移交给退款智能体。"
...
|------|--------|------|
| 分诊模式 | ✅ | 前线智能体路由用户到专家 |
| 基于技能的移交 | ✅ | "任务需要代码→移交编码者" |
| 短对话 | ✅ | 客服、FAQ→工单、简单工作流 |
| 长会话+共享内存 | ❌ | 移交重置对话状态 |
| 并行执行 | ❌ | 移交是一对一的——活跃智能体切换 |
| 审计和重放 | ❌ | 无状态运行难以精确重放 |
...
| 维度 | Swarm | GroupChat |
|------|-------|-----------|
| 路由决策 | 当前智能体通过调用移交工具 | 选择器（函数或 LLM）从外部 |
| 决策位置 | 活跃智能体的工具调用中 | GroupChatManager 中 |
| 概念 | "智能体决定下一步" | "管理者决定下一步" |

### 2.6 OpenAI Agents SDK 的生产改进
...
### 4.1 OpenAI Agents SDK vs Swarm
...
- **Swarm 的最小 API 在中文团队中更容易采用**——学习成本低
- **中文客服场景特别适合 Swarm**——分诊→退款/销售/支持的模式通用
- **移交日志用中文写**——方便中文团队审查
...
**参考答案：**
**例程**：定义智能体角色和可用工具的系统提示词。**移交**：智能体可以调用的工具，返回新 Agent 对象。运行时检测 Agent 返回值并切换活跃智能体。

### Q2：Swarm 的无状态权衡意味着什么？（难度：⭐⭐）

**参考答案：**
Swarm 在运行之间不持久化任何东西。内存、连续性、长期任务都是调用者的问题。OpenAI Agents SDK 添加了会话状态来解决这个问题。Swarm 本身保持无状态——这是概念清晰性的代价。

### Q3：Swarm 移交和 GroupChat 选择器的区别？（难度：⭐⭐⭐）
...
| 术语 | 人们怎么说 | 实际含义 |
|------|-----------|---------|
| 例程 (Routine) | "智能体提示词" | 系统提示词 + 工具列表。定义角色和可用移交 |
| 移交 (Handoff) | "转移到另一个智能体" | 活跃智能体可以调用的工具，返回新 Agent |
| 无状态 (Stateless) | "运行间无内存" | Swarm 不持久化任何东西；内存是调用者的问题 |
| 活跃智能体 | "谁在发言" | 当前持有对话的智能体。移交改变这个 |
| 移交过滤器 | "移交时传输什么" | 控制移交时传输什么上下文的 SDK 功能 |
...
4. **【思考】** 对比 Swarm 移交和 GroupChatManager 选择器。哪种模式使提示注入更糟，为什么？
...
4. [文档] Anthropic. "Claude Code Subagents". https://docs.anthropic.com/en/docs/claude-code — Claude Code 子智能体的移交模式
