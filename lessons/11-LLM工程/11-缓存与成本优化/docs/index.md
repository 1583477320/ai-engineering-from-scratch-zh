...
### 错误 1：未监控 Prompt Cache 命中率

**现象：** 以为 Prompt Cache 在生效但实际没有——成本没有下降。

**原因：** 如果系统消息每次调用都有微小变化（如时间戳），缓存会失效。

**修复：** 监控缓存命中率——在 API 响应中检查 `cached_tokens` 字段。确保系统消息完全不变。

### 错误 2：响应缓存过期策略不当

**现象：** 缓存了过时的信息——用户看到旧数据。

**原因：** 缓存没有设置合理的 TTL——昨天的价格信息今天还在返回。

**修复：** 根据数据更新频率设置 TTL：实时数据=0，每日更新=86400秒。
...
| 产出 | 文件 | 说明 |
|------|------|------|
| 缓存与成本分析 | `code/main.py` | 缓存命中率分析 + 月度成本估算 |
