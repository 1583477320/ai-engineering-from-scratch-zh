...
**现象：** 用户问"北京天气"，LLM 调用了 `search_web` 而非 `get_weather`。

**原因：** 工具描述太笼统——没有明确说明何时使用哪个工具。

**修复：** 在工具描述中写"当用户询问[具体场景]时使用此工具"——明确使用条件。

### 错误 2：参数格式错误

**现象：** LLM 生成了有效 JSON 但参数类型错误（如字符串而非数字）。

**原因：** 工具的参数 schema 中类型定义不够严格。

**修复：** 在 schema 中添加 `enum` 约束和 `minimum/maximum` 约束。
...
| 产出 | 文件 | 说明 |
|------|------|------|
| 函数调用实现 | `code/main.py` | 工具定义 + 执行 + 多轮调用循环 |
