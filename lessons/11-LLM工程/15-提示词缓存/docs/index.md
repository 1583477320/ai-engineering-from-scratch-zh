...
### 错误 1：系统消息包含动态内容导致缓存失效

**现象：** Prompt Cache 命中率为 0——成本没有下降。

**原因：** 系统消息中包含时间戳、随机数等动态内容——每次调用前缀都不同。

**修复：** 将动态内容移到用户消息中。系统消息应只包含静态内容（角色、规则、工具描述）。

### 错误 2：未利用 Prompt Cache 设计提示词结构

**现象：** 有 Prompt Cache 功能但没有利用——成本未降低。

**原因：** 提示词结构不正确——重要的静态内容被放在每次变化的部分。

**修复：** 按以下顺序组织提示词：静态系统消息（缓存）→ 动态对话历史 → 当前查询。
...
| 产出 | 文件 | 说明 |
|------|------|------|
| 缓存分析工具 | `code/main.py` | Prompt Cache 命中率分析 + 成本节省估算 |
