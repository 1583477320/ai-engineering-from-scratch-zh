...
**现象：** 查询"Python 数据库"无法匹配文档"如何查询 SQL 表"。

**原因：** 不同嵌入模型的向量空间不兼容——用 model-A 生成的查询向量和 model-B 生成的文档向量无法直接比较余弦相似度。

**修复：** 始终用同一个模型生成所有向量。迁移模型时需要重新嵌入所有文档。

### 错误 2：忽略文档预处理

**现象：** 嵌入结果质量差——相似文档得分低。

**原因：** 原始文档包含 HTML 标签、特殊字符、多余空白——这些噪声被编码进向量。

**修复：** 嵌入前做标准化清洗——去 HTML 标签、统一空白、截断超长文档。
...
| 产出 | 文件 | 说明 |
|------|------|------|
| 向量搜索实现 | `code/main.py` | 简单向量数据库 + 余弦相似度搜索 |
