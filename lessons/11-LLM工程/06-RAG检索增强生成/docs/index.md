...
**现象：** 大块导致检索不精确，小块导致上下文不完整。

**原因：** 分块太大时，块内混合了多个主题——检索到了块但相关部分只有 10%。分块太小时，每个块缺乏足够的上下文。

**修复：** 256-512 token 的块大小是最佳起点。使用 10-50 token 的重叠确保跨块语义连续。

### 错误 2：向量搜索 + 关键词搜索未融合

**现象：** 纯向量搜索漏掉了包含精确关键词的文档。

**原因：** 向量搜索擅长语义但不擅长精确匹配——"MySQL 5.7 版本"可能被匹配到"PostgreSQL"。

**修复：** 使用混合搜索（Hybrid Search）——RRF 融合向量搜索和 BM25 结果。

### 错误 3：RAG 提示词不强调检索内容

**现象：** 模型用自己的知识回答，忽略检索到的文档。

**原因：** 提示词中检索内容被放在不显眼的位置，或没有明确指示"基于以下文档"。

**修复：** 在提示词中明确写"基于以下文档回答，如果文档中没有相关信息就直接说不知道"。
...
| 产出 | 文件 | 说明 |
|------|------|------|
| RAG 管道实现 | `code/main.py` | 向量存储 + 检索 + 生成的完整 RAG |
