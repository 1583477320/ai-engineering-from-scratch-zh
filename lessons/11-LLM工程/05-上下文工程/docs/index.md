...
**原因：** LLM 存在"Lost in the Middle"效应——对上下文中间部分的注意力权重最低。

**修复：** 将最重要的信息放在开头或结尾。中间区域放不那么关键的背景信息。

### 错误 2：对话历史无限增长

**现象：** 长对话后响应速度变慢、成本上升、质量下降。

**原因：** 对话历史随轮次线性增长——20 轮对话可能已有 10K+ token。

**修复：** 使用滑动窗口保留最近 5-10 轮 + 历史摘要。或者用摘要压缩策略。
...
| 产出 | 文件 | 说明 |
|------|------|------|
| 对话管理器 | `code/main.py` | 滑动窗口 + 历史摘要的上下文管理 |
