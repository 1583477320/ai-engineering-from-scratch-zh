...
### Q1：音频分类中，"类别不平衡"和"领域偏移"分别是什么问题？（难度：⭐⭐）

**参考答案：**
类别不平衡：AudioSet 有 632 类，头部类（"Speech"）有 200 万片段，尾部类（"Scissors"）只有几百个——100,000:1 的长尾。直接训练会被多数类主导，少数类召回率为零。解决方案：平衡采样、Focal Loss、类别权重。

领域偏移：训练数据是干净的录音室环境——但部署在嘈杂的街道或车内。信噪比、混响、距离差异导致特征分布完全不同——模型在训练集上 95% 准确率，部署后只有 70%。解决方案：数据增强（Mixup、SpecAugment）、领域自适应。

### Q2：为什么 BEATs 微调比从零训练 CNN 更好？（难度：⭐⭐）

**参考答案：**
三个原因：**（1）自监督预训练捕捉了音频的通用结构**——BEATs 在数百万小时上学习了频率模式、节奏、音色的表示，这些通用特征迁移到任何下游任务。**（2）样本效率**——自监督预训练后只需 1-10% 的监督数据就能达到 SOTA 的 95%，从零训练需要 100% 的数据才能开始学习特征。**（3）计算效率**——BEATs-iter3 用 1/4 计算量超越了从零训练的 AST，因为它不需要从随机初始化开始。
