...
- **阶段 06 · 02（频谱图）→** 编解码器的输入正是第 02 课的梅尔频谱图——80 梅尔是 EnCodec 的标准输入
- **阶段 06 · 09（音乐生成）→** MusicGen 的 token 就是 EnCodec 编解码器产生的 token——本课是 MusicGen 工作原理的技术基础
- **阶段 05 · 19（子词分词）→** 编解码器的"codebook"概念与 BPE 的"词表"概念类似——都是离散化连续信号
...
**现象：** 用 EnCodec 从文本生成语音——结果模糊，说话人识别困难。

**原因：** 纯重建编解码器（EnCodec/DAC）将所有信息（语言+声学）压缩到同一个 codebook 中。当 LLM 需要从文本生成 codec token 时，它必须同时学习语言结构和声学结构——这不可扩展。

**修复：** 在语音生成场景中使用语义-声学分离的编解码器（Mimi）——codebook 0 编码语义内容，codebooks 1-7 编码声学细节。

### 错误 2：忽视帧率对 LM 成本的影响

**现象：** 用 EnCodec（75Hz）训练 LM，推理时比预期慢 6 倍。

**原因：** 帧率直接决定序列长度——1 秒音频 EnCodec 产生 75 token，Mimi 产生 12.5 token。更长序列 = 更多 LM 推理步数。

**修复：** 在评估 LM 方案时先确认帧率。Mimi 的 12.5Hz 帧率对流式语音生成特别有吸引力。

---

## 5. 面试考点

### Q1：RVQ 为什么能用 8 个小 codebook 达到比单个大 codebook 更高的质量？（难度：⭐⭐）

**参考答案：** RVQ 的关键洞察是渐进式残差量化——第一个 codebook 量化原始信号的"粗轮廓"，后续 codebook 量化残差。8 个 1024 码的 codebook 理论词汇量为 1024^8 = 10^24。实际中每个 codebook 的设计可以针对不同的信号层次——codebook 0 关注语义，codebook 1-3 关注时序细节，codebook 4-7 关注高频成分。这种分层设计比单个大 codebook 在相同码率下质量更高

---

## 📚 小结

神经音频编解码器是 LLM 处理语音/音乐的前提——它们将连续波形转换为离散 token 序列。RVQ 是核心技术——用一系列小 codebook 级联替代单个大 codebook。2026 年两个关键趋势：(1) 帧率越低越好（Mimi 12.5Hz vs EnCodec 75Hz = 6 倍 LM 效率提升）；(2) 语义-声学分离——codebook 0 编码语言内容，其余编码声学细节，让 LLM 无需同时学习两种结构
...
| 产出 | 文件 | 说明 |
|---|---|---|
| 编解码器选择提示词 | `outputs/skill-codec-picker.md` | 按任务选择编解码器和配置方案 |
